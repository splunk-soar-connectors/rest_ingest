...
def handle_request(request, path_parts):
    ...
    response = _call_phantom_rest_api(
        request, "asset", "get", params={"_filter_name": _quote_wrap(asset_name), "_filter_disabled": _quote_wrap(False)}
    )
    ...
    try:
        ...
        if not hasattr(result, "__iter__"):
            return HttpResponse('Parse script returned an invalid response of type "{}"'.format(type(result)), status=400)  # nosemgrep
        ...
        for r in result:
            if not hasattr(r, "get"):
                return HttpResponse(  # nosemgrep
                    'Parse script returned an invalid response containing a(n) "{}" object'.format(type(r)), status=400
                )
            ...
            if container and hasattr(container, "__setitem__"):
                ...
                if not container_id:
                    return HttpResponse(  # nosemgrep
                        "Unknown error when inserting container, no resulting container id. Response: {}".format(response_json), status=400
                    )
            ...
            if artifacts and hasattr(artifacts, "__iter__"):
                for j, artifact in enumerate(artifacts):
                    ...
                    if response_json.get("success", False) is True:
                        ...
                        return HttpResponse("Unknown error when inserting artifact. Response: {}".format(response_json), status=400)  # nosemgrep
